...
the resulting DITR schemes are multi-step methods that are
different from the common implicit linear multi-step 
methods such as
...
\begin{equation}
    \omega_f^G = \sqrt{S^{\frac{1}{d-1}}D_f^{-1}}
\end{equation}
where $d$ is the number of spatial  dimension, $S$ is the area of the interface,
and $D_f$ is the distance between barycenters of the two cells. 
This form of
...
implicit linear multi-step method also use the direct
...
when applied to spatial  discretized flow problems, and
...
Some difference scheme on uniform grid is used as spatial  discretization:
\begin{equation}
...
apart from the first-order upwind spatial  discretization,
