...
\cite{wang2017compact_VR}. As a special case of
SDIRK, ESDIRK methods are SDIRK with an explicit first stage,
...
\begin{subequations}
    ...
    \end{align}
\end{subequations}
where $t^{n,i}=t^n+c_i\inc t$, and $c_1 = 0, c_3 = 1$.

The only stage value besides endpoint is $\uu^*$.
...
a numeric integration on 
$t^{n,1},t^{n,2},t^{n,3}$, which demands:
\begin{equation}
    \begin{aligned}
