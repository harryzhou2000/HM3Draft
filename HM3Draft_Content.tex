...
that limits their physical time step sizes
...
discussed efficient implementation of the dual time stepping
...
The compressible Navier-Stokes equations in the conservative form are
\begin{equation}
    \label{eq:NS}
...
When $\mu=0$,
$\F_v=0$ and
\eeqref{eq:NS} becomes Euler equation.
...
the discretization of \eeqref{eq:NS} can therefore lead 
...
The piecewise polynomial approximation \eeqref{eq:FVRec} does not
...
As the spatial terms are discretized in \eeqref{eq:FVInt},
...
of average values $\UM_i$, \eeqref{eq:FVInt} can be rearranged into the
assembled form
\begin{equation}
    \label{eq:FVODE}
    \frac{d\uu}{dt} = \R(t, \uu)
...
isotropic.
The coefficients in \eeqref{eq:wdHQMOPT} are taken from previous research \cite{huang2022high}, which were
...
This form of
geometric weight is also taken from previous research \cite{huang2022high}.
...
The local coefficient matrices and vectors \eeqref{eq:vrCoeffs} can be
...
boundary conditions can be found in the original paper \cite{wang2017compact_VR}.
...
The first order ODE \eeqref{eq:FVODE} arising
...
methods for solving \eeqref{eq:FVODE}
using a straightforward construction procedure.

Direct integration of \eeqref{eq:FVODE}
...
In order to acquire the integration result in
\eeqref{eq:DI}, a numerical quadrature rule in the interval
...
The numerical integration process used in \eeqref{eq:Quad3}
...
% is unknown, thus an approximation is required for numerical calculation.
...
Combining the direct integration approximation in \eeqref{eq:Quad3}
and a temporal reconstruction approximation in \eeqref{eq:TR},
...
variants of temporal reconstruction in \eeqref{eq:TR}.
...
\eeqref{eq:Quad3} yields a
truncation error of $O((\inc t)^{m+2} )$ expressed in \eeqref{eq:Quad3Err}
\begin{equation}
    \label{eq:Quad3Err}
    \begin{aligned}
        \uu^{n+1} - \uu^{n} = & \int_{t^n}^{t^{n+1}}{
...
\eeqref{eq:TR} has a truncation error of $O((\inc t)^{l+1})$
...
Substituting \eeqref{eq:TRErr} into the $t^{n+c_2}$ term
...
$\uu^{n+c_2}$ is determined by the temporal reconstruction of \eeqref{eq:TR}.
\eeqref{eq:fullLTE} assumes
...
The following subsections will illustrate some
practical variants of DITR methods based on \eeqref{eq:Quad3} and \eeqref{eq:TR}.


\subsection{Variants of DITR methods}

The following subsections will derive several special versions of DITR based on 
...
combined with direct integration \eeqref{eq:Quad3}
...
When $c_2=1/2$, quadrature rule in  \eeqref{eq:Quad3} has precision
...
\eeqref{eq:interpU2R2} and \eeqref{eq:Quad3}
...
determined by \eeqref{eq:interpU2R2} and \eeqref{eq:integ0},
...
based on the formulae provided in textbook \cite{wanner1996solving}.
...
Analysis on \eeqref{eq:stabilityFuncU2R2}
...
The temporal reconstruction \eeqref{eq:TRU2R1}
combined with direct integration \eeqref{eq:Quad3}
forms the DITR U2R1 method.

The interpolation bases shown in \eeqref{eq:interpU2R1}
are quadratic.
...
The temporal reconstruction \eeqref{eq:TRU3R1}
combined with direct integration \eeqref{eq:Quad3}
...
\begin{equation}
    ...
    \label{eq:DISum}
\end{equation}
with weights decided by \eeqref{eq:integ0},
...
dual time stepping technique, which will be explained in 
Section \ref{sssec:numScan}.
...
% as described in literature \cite{kulikov2006familyNIRKOrig}.
...
%     \frac{d\uu}{dt} = \R(t, \uu)
% \end{equation*}
% which is directly taken from  \eeqref{eq:FVODE}.
% A direct integration of  \eeqref{eq:FVODE}
% leads to a time-marching relation
% \begin{equation}
%     \uu^{n+1} = \uu^{n} + \int_{t^n}^{t^{n+1}}{
%     \R(t, \uu)
...
% The Hermite interpolation \eeqref{eq:Hermite}
...
% The quadrature rule \eeqref{eq:Quad} has truncation error
% $O(\inc t^3)$ at least,
...
% calculated with \eeqref{eq:HM3-2}, which is derived
...
%     \label{eq:interp}
% \end{equation}
% The first equation \eeqref{eq:HM3-1} is
% a numerical integration on interval $[t^n,t^{n+1}]$ with
% polynomial nodes
...
%     \label{eq:integ}
% \end{equation}
% The Hermite interpolation gives \eeqref{eq:HM3-2}
% local truncation error $O(\inc t^4)$,
% and the numerical integration gives \eeqref{eq:HM3-1}
% local truncation error $O(\inc t^3)$, therefore the
% classic order of accuracy of HIRK \eeqref{eq:HM3} is
% 3. If $c_2=1/2$, then the numerical integration \eeqref{eq:HM3-2}
...
% Reformulating \eeqref{eq:HM3} into
...
%               & $b_1$          & $b_2$    & $b_3$
%     \end{tabular}
%     \caption{Butcher tableau of \eeqref{eq:HM3}}
%     \label{tab:HM3Butcher}
% \end{table}

% From table \ref{tab:HM3Butcher} with the coefficients
% decided with \eeqref{eq:interp} and \eeqref{eq:integ},
% one can find that the 4th-order accurate
...
% the simplifying assumptions, which is a trivial procedure
% given the formulae provided in literature{wanner1996solving}.
...
% described in literature \cite{kulikov2006familyNIRKOrig}.
...
% Further analysis on \eeqref{eq:stabilityFunc}
...
consist of two implicit equations \eeqref{eq:DISum} and
\eeqref{eq:TRSum}, which are results of direct integration
...
\eeqref{eq:abstractImplicit} can be written as 
$\FF(\uu^{n+1})=0$.
For example, for trapezoid rule (Crank-Nicolson method),
\eeqref{eq:abstractImplicit} reads
\begin{equation}
    \label{eq:trapF}
    \FF(\uu^{n+1}) =
    ...
    =0
\end{equation}
Generally, $\FF^s$ should be arranged so that
$\R(\uu^s)$ in $\FF^s$ has coefficient being $O(1)$ and positive,
while $\uu^s$ has its coefficient being $O(\inc t^{-1})$ and negative.
With the idea of dual time stepping, namely
\begin{equation}
    \frac{\dd\uu^{s}}{\dd \tau} = \FF^s(\uu^{s})
    \label{eq:exactDualTime}
\end{equation}
combined with a backward Euler method on the
pseudo time $\tau$, \eeqref{eq:abstractImplicit}
is solved with
\begin{equation}
...
When $\inc\tau\rightarrow0$, Equation \eeqref{eq:abstractNewton}
tends to reflect the exact evolution of dual time stepping
\eeqref{eq:exactDualTime}.
...
\end{equation}
can be substituted in \eeqref{eq:fd} and produces
a relation of
\begin{equation}
...
For the finite difference scheme of \eeqref{eq:fd},
...
stepping scheme of \eeqref{eq:exactDualTime} can be
...
With the substitution of \eeqref{eq:fd},
\eeqref{eq:canonicalFF},
and
\eeqref{eq:simpleWaveTestTau}
into the exact pseudo time evolution \eeqref{eq:exactDualTime},
...
\eeqref{eq:canonicalFF}, it is guaranteed that $\imag (\kappa^*) < 0$,
...
thus \eeqref{eq:abstractNewton} can be applied directly.
...
are present in both \eeqref{eq:DISum} and \eeqref{eq:TRSum}.
Note that in \eeqref{eq:DISum} and \eeqref{eq:TRSum},
$\R^{n+1},\R^{n+c_2}$ are also dependent on $\uu^{n+1},\uu^{n+c_2}$.
Assuming $\R$ to be nonlinear, linear combinations of
\eeqref{eq:DISum} and \eeqref{eq:TRSum} are
...
namely \eeqref{eq:abstractNewton}, 
...
The huge advantage of this iteration method is 
...
\eeqref{eq:abstractNewtonNestedNoTau}
...
analysis based on pseudo time introduced in 
Section \ref{sssec:fourier} is conducted.
...
\begin{equation}
    ...
    \label{eq:fdup1}
\end{equation}
\eeqref{eq:nestedDITR_F} and \eeqref{eq:canonicalFF} lead to:
\begin{equation}
    \begin{aligned}
...
Once again simple wave solution of \eeqref{eq:simpleWaveTestTau}
is applied in \eeqref{eq:nestedDITR_F2}, and comparing with
\eeqref{eq:kappaSDefTau}, it is found that
\begin{equation}
    \begin{aligned}
        \kappa^* = & \imagUnit
        \Biggl[
...
and $\CFLt$, which is illustrated in \figref{fig:HM3_U2R2050_KappaS}.
\figref{fig:HM3_U2R2050_KappaS} indicates for DITR U2R2 $c_2=1/2$,
...
\eeqref{eq:DITRFFDef} and \eeqref{eq:DITRNaiveDualTime} 
...
to avoid solving \eeqref{eq:DITRNaiveDualTime} directly.
To this end, \eeqref{eq:DITRNaiveDualTime} can be solved 
...
In the first stage of iteration in \eeqref{eq:DITRSDS},
...
Compared to the updating procedure of fully coupled iteration method,
...
For preconditioner $\Pm$ of \eeqref{eq:specialP},
...
Therefore, 
to ensure the stability of \eeqref{eq:ode-stage-decouple-fourier},
...
DITR U2R2 $c_2=1/2$ in \figref{fig:MuU2R2} with various $\beta$.
...
\eeqref{eq:FVODE} can be reformed into a system of decoupled linear scalar ODEs 
with it components in the form of
\begin{equation}
    \label{eq:odeScalarTest}
    \derivative{y}{t} = \lambda y
\end{equation}
\eeqref{eq:odeScalarTest} with $\real(\lambda)\leq 0$
...
The preconditioning parameter, $\beta$, as discussed in 
Section \ref{sssec:SDSFourier}
and Section \ref{sssec:GSStability}, 
influences convergence properties of the
stage-decoupled solving strategy.
With $\inc \tau\rightarrow0$, as discussed in Section \ref{sssec:SDSFourier},
...
When $\inc \tau\rightarrow+\infty$, $|G_{GS}|$ from Section \ref{sssec:GSStability}
...
\end{figure}
\figref{fig:MuGGSSearch} illustrates the numerical scanning
...
which is indicated in \figref{fig:HM3_U2R205_GGSRegions}.
In \figref{fig:HM3_U2R205_GGSRegions}, with the
...
in \figref{fig:MuGGSSearch} and Table \ref{tab:resrictionBetaSearch}.
From \figref{fig:MuGGSSearch} and Table \ref{tab:resrictionBetaSearch},
with $c_2=0.25$, U2R1 has less restriction on $\beta$ than with $c_2=0.5$.
Meanwhile, \figref{fig:HM3_U2R1_GGSRegions} illustrates the
...
shown in \figref{fig:HM3_U2R205_GGSRegions}.
...
The settings can be found in literature \cite{hu1999weighted_WENO}.
...
Results of large time step testing are shown in \figref{fig:IV10Step}.
Clearly, from \figref{sfig:IV10Step_BDF2}, BDF2 almost completely
...
the propagation direction as shown in \figref{sfig:IV10Step_HM3LBT},
while the DITR U2R2 $c_2=0.55$ inhibits them better in \figref{sfig:IV10Step_HM3}.
...
shown in \figref{fig:IVTests}.

\figref{sfig:IVTests_Conv} shows that with the same time step size,
...
As of order of error, in \figref{sfig:IVTests_Conv},
...
\subsection{Two-dimensional vortex shedding problem}
...
be illustrated with the errors measured by \eeqref{eq:vorstreetErr}.
...
Details about the initial and boundary settings may be found in literature \cite{woodward1984dmr}.
...
Analysis of the results of numerical tests, confirms
